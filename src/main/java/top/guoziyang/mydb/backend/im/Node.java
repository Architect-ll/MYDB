...
public class Node {
    ...
    /**
     * 创建一个新的空根节点（叶子节点）的原始字节数组，这个新的根节点没有子节点和键。
     */
    ...
    class SearchNextRes {
        long uid;   // 下一个节点的UID
        long siblingUid; // 下一个节点的兄弟节点的UID
    }

    /**
     * 在B+树的节点中搜索下一个节点的方法，寻找对应 key 的 UID, 如果找不到, 则返回兄弟节点的 UID
     */
    ...
    /**
     * 在B+树的节点中插入一个键值对，并在需要时分裂节点。(注意B+树的扩展机制)
     */
